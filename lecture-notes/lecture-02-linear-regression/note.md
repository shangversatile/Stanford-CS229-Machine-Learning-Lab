...
$$
h_\theta(x)=\theta_0+\sum_{j=1}^{n}\theta_jx_j
...
若限制 $\lVert\Delta\rVert_2=\varepsilon$，根据 Cauchy--Schwarz inequality，

$$
\nabla J(\theta)^T\Delta
\geq-\lVert\nabla J(\theta)\rVert_2\lVert\Delta\rVert_2
=-\varepsilon\lVert\nabla J(\theta)\rVert_2.
$$
...
处取得。因此 negative gradient 是在 local first-order approximation 和 Euclidean norm 约束下的 steepest descent direction。换用不同 norm 或 geometry，steepest direction 也会改变。
...
$$
J_i(\theta)
=\frac12\left(h_\theta(x^{(i)})-y^{(i)}\right)^2.
$$
...
=\frac{\partial (\frac12e_i^2)}{\partial e_i}
\cdot
...
\begin{aligned}
\nabla_\theta J(\theta)
&=\frac12\left(2X^TX\theta-2X^Ty+0\right)\\
&=X^TX\theta-X^Ty\\
&=X^T(X\theta-y).
\end{aligned}
$$
