...
(AB)^T\in\mathbb{R}^{r\times p}.
$$

另一方面，

$$
B^T\in\mathbb{R}^{r\times q},
\qquad
A^T\in\mathbb{R}^{q\times p},
$$
...
若限制 $\lVert\Delta\rVert_2=\varepsilon$，则 Cauchy--Schwarz inequality 给出
...
* $\nabla f$ 指向 local first-order increase 最大的方向；
* $-\nabla f$ 指向 local first-order decrease 最大的方向；
* gradient 与 contours orthogonal，因此 gradient descent 会跨越 contours，而不是沿着同一 loss level 移动。
